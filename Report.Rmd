...
# Lions vs Vikings Real-Life Scenario
input_63 <- data.frame(
  "yardline_100" = 72,
...
# Lions vs Vikings Real-Life Scenario, but with a run
input_64 <- data.frame(
  "yardline_100" = 72,
  "half_seconds_remaining" = 248,
  "game_half" = "Half2",
...
The following table illustrates all scenarios we ran and the outputs:
...
By using NFL Play-By-Play data, we have successfully built a model that can assist NFL coaches and players in knowing if a fourth down conversion should be attempted. We believe the function we've created is incredibly useful for coaches because it is a simple way to conduct an on-the-fly analysis of the decision whether to attempt a fourth down conversion or not. 

The model is not terribly accurate, with a kappa of only around 0.45, mostly due to the selection bias we've discussed above. Furthermore, the model has a weakness because it does not account for field goal outcomes when recommending a fourth down conversion with positive but fewer than 3 expected net points. All of the models are reasonably accurate and provide valuable insight with varying levels of interpretability. The most accurate model is TODO

This analysis is important to coaches, players, and fans for many reasons. Money and resources should be spent to further refine these models because this project can help football decision makers appropriately determine the best course of action which could then lead to winning the game.
