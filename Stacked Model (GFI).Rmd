...
GFI_GLM_ALL <- select(GFI_GLM_ALL, 
  season_type, 
  week, 
  posteam_type, 
  yardline_100, 
...
glm_pred <- predict(model_glm, newdata = gfi_glm_validation)
glm_pred <- ifelse(glm_pred < 0, 0, 1)
glm_pred <- as.factor(glm_pred)

library(caret)
confusionMatrix(glm_pred, as.factor(gfi_glm_validation$fourth_down_converted))
```
